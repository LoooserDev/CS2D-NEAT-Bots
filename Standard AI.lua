...
dofile("bots/includes/neat.lua")		-- neuroevolution of augmenting topologies 

---NEAT Settings
vai_neat = fai_neat_init(4,5)
fai_neat_initializePool(vai_neat)
fai_neat_load(vai_neat, "bots/savefile.sf")
vai_neat_engage_timeout=200
...
for i=1,32 do
	...
	vai_aim_rotation[i]=0
	vai_aim_off_angle[i]=0
	vai_aim_distance[i]=0
	vai_engage_timer[i]=vai_neat_engage_timeout
	vai_start_score[i]=0
	vai_fitness[i]=0
end
...
function ai_onspawn(id)
	...
	vai_aim_rotation[id]=math.random(0,359)
	vai_aim_off_angle[id]=0
	vai_aim_distance[id]=math.random(0,315)
	vai_engage_timer[id]=vai_neat_engage_timeout
	--vai_start_score[id]= player(id,"score")
	if vai_fitness[id]~=0 then
